...
class restore_videotrack_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $paths[] = new restore_path_element('videotrack_progress', '/activity/videotrack/progresses/progress');
    ...
    protected function process_videotrack($data) {
        ...
        $data->displaymode = $data->displaymode ?? 0;
        $data->focusmode = $data->focusmode ?? 0;
        $data->preventforward = $data->preventforward ?? 0;
    ...
    /**
     * Restore a learner progress record when user data is included in the backup.
     *
     * @param array $data Progress data.
     */
    protected function process_videotrack_progress($data) {
        global $DB;
        ...
        $data->watchedsegments = $data->watchedsegments ?? null;
        $data->duration = $data->duration ?? 0;
        $data->lastposition = $data->lastposition ?? ($data->highesttime ?? 0);
        $data->lastheartbeat = 0;
        $data->timecreated = $this->apply_date_offset($data->timecreated);
        $data->timemodified = $this->apply_date_offset($data->timemodified);
        $DB->insert_record('videotrack_progress', $data);
    }
    ...
    protected function after_execute() {
        ...
        $this->add_related_files('mod_videotrack', 'captions', null);
    }
}
