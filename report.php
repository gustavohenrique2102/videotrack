...
$PAGE->set_url('/mod/videotrack/report.php', ['id' => $cm->id, 'page' => $page]);
...
$users = get_enrolled_users($context, 'mod/videotrack:view', 0, 'u.*', 'u.lastname, u.firstname', 0, 0, true);
...
if (empty($users)) {
    ...
    $users = array_slice(array_values($users), $page * $perpage, $perpage, true);
    ...
    $table->head = [
        ...
        get_string('progressupdated', 'mod_videotrack'),
    ];
    ...
    foreach ($users as $user) {
        ...
        $studentname = $userpicture . ' ' . html_writer::span($fullname, 'ml-2 ms-2');
        $studentcell = html_writer::div($studentname, 'd-flex align-items-center');
        ...
        <div class="d-flex align-items-center" style="min-width: 200px;">
            <div class="progress w-100 mr-2 me-2" style="height: 16px; border-radius: 8px;">
                <div class="progress-bar progress-bar-striped bg-primary" ' .
                'role="progressbar" style="width: ' . $percent . '%;" ' .
                'aria-valuenow="' . $percent . '" ' .
                'aria-valuemin="0" aria-valuemax="100"></div>
            </div>
            <span class="font-weight-bold fw-bold">' . $percent . '%</span>
        </div>';

        // Completion status is derived from the current threshold so reports stay correct if the teacher changes it.
        $iscompleted = $videotrack->targetpercent > 0 && $percent >= (int)$videotrack->targetpercent;
        if ((int)$videotrack->targetpercent <= 0) {
            $completedcell = html_writer::span(get_string('notapplicable', 'mod_videotrack'), 'text-muted');
        } else if ($iscompleted) {
            $completedcell = html_writer::span(
                '<i class="fa fa-check-circle text-success mr-1"></i> ' . get_string('yes', 'moodle'),
                'text-success font-weight-bold fw-bold'
            );
        } else {
            $completedcell = html_writer::span(
